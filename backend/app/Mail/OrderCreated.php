...
class OrderCreated extends Mailable
{
    ...
    public function envelope(): Envelope
    {
        ...
        // Если тип не установлен, по умолчанию считаем разовым
        $clientType = $this->order->client_type ?? ($this->order->client->client_category ?? 'one_time');
        $isOneTimeClient = $this->order->client && ($clientType === 'one_time' || $clientType === 'one-time' || $clientType === null);
    ...
    public function content(): Content
    {
        ...
        // Если тип не установлен, по умолчанию считаем разовым (безопаснее - всегда показывать оплату)
        $clientType = $this->order->client_type ?? ($this->order->client->client_category ?? 'one_time');
        $isOneTimeClient = $this->order->client && ($clientType === 'one_time' || $clientType === 'one-time' || $clientType === null);
